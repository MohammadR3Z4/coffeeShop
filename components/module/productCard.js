...
function ProductCard( props ) {
    ...
    return (
        <div className='lg:p-5 p-4 rounded-2xl bg-white shadow-[0_1px_10px_0px_rgba(0,0,0,0.05)] font-Vazir'>
            <div className={`${item.Isdiscount ? "absolute" : "hidden"} text-center flex justify-center items-center bg-Orange-300 
            md:w-[54px] md:h-[30px] w-10 h-5 text-white rounded-[100px]`}>
                <span className={`md:text-base text-xs font-semibold`}> {item.discount}% </span>
            </div>
            <div className='flex justify-center items-center'>
                <Link href={`/Store/${item.id}`}>
                    <Image className='md:w-auto md:h-auto w-[128px] h-[128px]' src={item.image} width={260} height={260} alt='Product'/>
                </Link>
            </div>
            <div>
                <Link href={`/Store/${item.id}`} className='text-Zinc-700 md:text-xl text-sm font-medium mt-5 hover:underline'> {item.tittle} </Link>
                <div className='flex flex-wrap gap-2 mt-[10px]'>
                    <div className='flex justify-center order-2 items-center'>
                        <p className={`${item.Isavailable ? "flex justify-center items-center" : "hidden"} ${item.Isdiscount ? "text-Gray-400 order-2 line-through me-0 decoration-Red-400" : "text-Teal-600 lg:text-xl font-semibold"} `}> 
                        {item.price.toLocaleString()}  
                            <span className={`${item.Isdiscount ? "ms-0 hidden" : "ms-1"} md:text-sm md:flex justify-center items-center text-xs font-normal no-underline`}> تومان </span> 
                        </p>
                    </div>
                    <p className={`${item.Isavailable ? "hidden" : "flex"} text-Red-400 lg:text-xl md:text-lg text-base font-normal`}> فعلا موجود نیست 
                    </p>
                    <div>
                        <p className={`${item.Isdiscount ? "flex text-Teal-600 lg:text-xl md:text-lg text-base font-semibold order-1" : "hidden"} `}> 
